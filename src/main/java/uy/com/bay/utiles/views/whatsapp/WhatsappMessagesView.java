...
public class WhatsappMessagesView extends VerticalLayout {
    ...
    private HorizontalLayout createTopBar() {
        fromDate = new DateTimePicker("fromDate");
        toDate = new DateTimePicker("toDate");
    ...
    private void configureGrid() {
        ...
        Grid.Column<WhatsappFlowTask> createdColumn = grid.addColumn(task -> formatDate(task.getCreated()))
                .setHeader("Created").setSortable(true).setComparator(WhatsappFlowTask::getCreated).setResizable(true).setKey("created");

        Grid.Column<WhatsappFlowTask> statusColumn = grid.addColumn(WhatsappFlowTask::getStatus)
                .setHeader("Status").setSortable(true).setResizable(true).setKey("status");

        Grid.Column<WhatsappFlowTask> scheduleColumn = grid.addColumn(task -> formatDate(task.getSchedule()))
                .setHeader("Schedule").setSortable(true).setComparator(WhatsappFlowTask::getSchedule).setResizable(true).setKey("schedule");

        Grid.Column<WhatsappFlowTask> processedDateColumn = grid.addColumn(task -> formatDate(task.getProcessedDate()))
                .setHeader("Processed Date").setSortable(true).setComparator(WhatsappFlowTask::getProcessedDate).setResizable(true).setKey("processedDate");

        Grid.Column<WhatsappFlowTask> toColumn = grid.addColumn(WhatsappFlowTask::getTo)
                .setHeader("To").setSortable(true).setResizable(true).setKey("to");
        ...
        Grid.Column<WhatsappFlowTask> wamidColumn = grid.addColumn(WhatsappFlowTask::getWamid)
                .setHeader("Wamid").setSortable(true).setResizable(true).setKey("wamid");
    ...
    private boolean filter(WhatsappFlowTask task) {
        ...
        boolean matchesStatus = matches(task.getStatus() != null ? task.getStatus().toString() : "", statusFilter.getValue());
        ...
        return matchesCreated && matchesStatus && matchesSchedule && matchesProcessedDate &&
               matchesTo && matchesTemplateName && matchesWamid && matchesResponseStatus;
    }

    ...
    private VerticalLayout createStep3() {
        ...
        upload.addSucceededListener(event -> {
            try (InputStream inputStream = buffer.getInputStream()) {
                ...
                List<ColumnMapping> selected = columnMappings.stream().filter(ColumnMapping::isToCode).collect(Collectors.toList());
                boolean allHeadersValid = true;
                for (ColumnMapping mapping : selected) {
                    String originalName = mapping.getNewName() != null && !mapping.getNewName().isEmpty() ? mapping.getNewName() : mapping.getOriginalName();
                ...
                if(allHeadersValid) {
                    ...
                    Notification.show("Error de validación: El archivo de mapeo de códigos no tiene el formato correcto.");
                }
    ...
    private VerticalLayout createStep4() {
        ...
        processButton.addClickListener(event -> {
            ...
            try {
                ...
                for (ColumnMapping mapping : selected) {
                    String columnName = mapping.getNewName() != null && !mapping.getNewName().isEmpty() ? mapping.getNewName() : mapping.getOriginalName();

...
public class ExpenseRequestTypeView extends Div implements BeforeEnterObserver {
	...
	public ExpenseRequestTypeView(ExpenseRequestTypeService expenseRequestTypeService) {
		...
		grid.setItems(query -> expenseRequestTypeService
		...
		grid.asSingleSelect().addValueChangeListener(event -> {
			if (event.getValue() != null) {
				UI.getCurrent()
						.navigate(String.format(EXPENSEREQUESTTYPE_EDIT_ROUTE_TEMPLATE, event.getValue().getId()));
			} else {
				clearForm();
	...
	public void beforeEnter(BeforeEnterEvent event) {
		Optional<Long> expenseRequestTypeId = event.getRouteParameters().get(EXPENSEREQUESTTYPE_ID)
				.map(Long::parseLong);
		if (expenseRequestTypeId.isPresent()) {
			Optional<ExpenseRequestType> expenseRequestTypeFromBackend = expenseRequestTypeService
					.get(expenseRequestTypeId.get());
			if (expenseRequestTypeFromBackend.isPresent()) {
				populateForm(expenseRequestTypeFromBackend.get());
			} else {
				Notification.show(
						String.format("El concepto solicitado no fue encontrado, ID = %s", expenseRequestTypeId.get()),
	...
	private void createEditorLayout(SplitLayout splitLayout) {
		...
		refund = new Checkbox("es devolución");
		formLayout.add(name, description, refund);

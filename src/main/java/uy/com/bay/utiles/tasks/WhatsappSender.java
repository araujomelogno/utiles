...
public class WhatsappSender {
	...
	public void sendMessages() {
		...
		List<WhatsappFlowTask> tasks = whatsappFlowTaskRepository.findAllByStatusAndScheduleBefore(Status.PENDING,
				new Date());
	...
	private void processTask(WhatsappFlowTask task) {
		try {
			...
			String responseBody = restClient.post().uri(url).header("Authorization", "Bearer " + apiToken)
					.contentType(MediaType.APPLICATION_JSON).body(task.getInput()).retrieve().body(String.class);
			...
			if (responseBody != null) {
				...
				// Also check standard Facebook structure just in case, as 'id' is usually

...
public class WhatsappService {
    ...
    public List<WhatsappTemplate> getTemplates() {
        ...
        try {
            WhatsappTemplatesResponse response = restClient.get()
                    .uri(url)
                    .header("Authorization", "Bearer " + apiToken)
                    .retrieve()
                    .body(WhatsappTemplatesResponse.class);
    ...
    @Transactional
    public InputStream processExcel(InputStream fileContent, boolean skipHeader, WhatsappTemplate selectedTemplate, Date schedule) throws IOException {
        Workbook workbook = new XSSFWorkbook(fileContent);
        Sheet sheet = workbook.getSheetAt(0);

        // Find the maximum column count to ensure we write the status in a consistent new column if desired,
